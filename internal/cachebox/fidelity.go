...
// Forget drops pair's counters entirely. Exposed for callers that want to
// bound registry growth across a long-running SDK process (design doc Q6:
// "keep the last completed pair queryable until the next preload/clear")
// -- not wired to an automatic trigger here, since the right moment is a
// control-plane-level decision this package doesn't have enough context to
// make on its own.
func (r *FidelityRegistry) Forget(pair PhasePair) {
	if r == nil {
		return
	}
	r.mu.Lock()
	delete(r.phases, pair)
	r.mu.Unlock()
}
